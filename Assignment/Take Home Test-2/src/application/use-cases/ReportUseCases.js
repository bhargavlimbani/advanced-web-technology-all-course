...
class ReportUseCases {
  async getOverdueBooks() {
    ...
    const overdueTransactions = transactions.filter(t => t.status === 'issued' && new Date(t.dueDate) < currentDate);
    
    // Enrich with book and user data
    const enrich = overdueTransactions.map(async (t) => {
      const book = await bookRepository.findById(t.bookId);
      const user = await userRepository.findById(t.userId);
      return { ...t, book, user };
    });

    return await Promise.all(enrich);
  }
